...
class UserConfig 
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('default_key_bindings', $value)) {
            $this->_usedProperties['defaultKeyBindings'] = true;
            $this->defaultKeyBindings = array_map(function ($v) { return new \Symfony\Config\PimcoreAdmin\User\DefaultKeyBindingsConfig($v); }, $value['default_key_bindings']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['defaultKeyBindings'])) {
            $output['default_key_bindings'] = array_map(function ($v) { return $v->toArray(); }, $this->defaultKeyBindings);
        }
