...
class SessionConfig 
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('attribute_bags', $value)) {
            $this->_usedProperties['attributeBags'] = true;
            $this->attributeBags = array_map(function ($v) { return \is_array($v) ? new \Symfony\Config\PimcoreAdmin\Session\AttributeBagsConfig($v) : $v; }, $value['attribute_bags']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['attributeBags'])) {
            $output['attribute_bags'] = array_map(function ($v) { return $v instanceof \Symfony\Config\PimcoreAdmin\Session\AttributeBagsConfig ? $v->toArray() : $v; }, $this->attributeBags);
        }
